...
if [ "${AUTO_CONFIG_ENABLED}" = "true" ]; then
  ...
  DATABASE_PROVIDER=${DATABASE_PROVIDER:-"$(get_cm_value sealos-system sealos-config databaseType)"}
  ...
  [ -n "${DATABASE_PROVIDER}" ] && HELM_ARGS="${HELM_ARGS} --set-string devboxConfig.databaseProvider=${DATABASE_PROVIDER}"
fi
