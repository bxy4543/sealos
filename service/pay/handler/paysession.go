...
func InsertDetails(client *mongo.Client, user, payMethod, amount, currency, orderID string, appID int64, details map[string]interface{}) error {
	...
	_, err = session.WithTransaction(context.Background(), func(sessCtx mongo.SessionContext) (interface{}, error) {
		...
		err := InsertPaymentDetails(client, user, payMethod, amount, currency, orderID, appID)
		if err != nil {
...
func InsertPaymentDetails(client *mongo.Client, user, payMethod, amount, currency, orderID string, appID int64) error {
