...
var QueryTmplMap = map[QueryTmplType]string{
	TrafficsNS:  `increase(sealos_nm_trafficstat_ipv4_egress_bytes_total{from_namespace="{{.Namespace}}, to_cilium_identity=~'(2|7)'"}[{{.IncreaseDuration}}])`,
	TrafficsAll: `increase(sealos_nm_trafficstat_ipv4_egress_bytes_total{to_cilium_identity=~'(2|7)'}[{{.IncreaseDuration}}])`,
}
