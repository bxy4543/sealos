...
func (p *prometheus) QueryNSTraffics(namespace string, queryParams QueryParams) (*resources.Metering, error) {
	...
	//	}
	//}
	return &resources.Metering{
		Value:    int64(matrixData[0].Values[0].Value),
		Category: namespace,
		Property: resources.NetWork,
...
func (p *prometheus) QueryAllNSTraffics(queryParams QueryParams) ([]*resources.Metering, error) {
	matrixData, err := p.queryTraffics("", queryParams)
	if err != nil {
		return nil, fmt.Errorf("query all ns traffics failed: %v", err)
	}
	var metering []*resources.Metering
	for _, v := range matrixData {
		...
		for i := range v.Values {
			metering = append(metering, &resources.Metering{
